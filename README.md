...
As checked on September 24, 2026, the provider's [current billing guide](https://docs.tokenfactory.nebius.com/other-capabilities/billing-new) requires a bank card to complete onboarding. An event promo code does not by itself establish a card-free account. This project's live-model validation is on hold while the event's activation route is clarified; `ENABLE_NEBIUS` remains off. The browser-only workflow and published validation below are available without an account.
